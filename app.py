from flask import Flask, render_template, request, redirect, url_for, session,jsonify,make_response,flash,abort,Response
...
class Professional_RegisterForm(FlaskForm):
    ...
    def validate_contact(self,contact):
        ...
        if user:
            raise ValidationError("Contact is already in use. Pick another one.")
...
class Registered_Professionals(db.Document):
...
def adminlogin():
    ...
    if form.validate_on_submit():
        ...
        if user and user.get_password(password):
            ...
            flash("Sorry, something went wrong.","danger")
    return render_template("adminlogin.html", title="Manager Login", form=form, adminlogin=True )
...
def register():
    ...
    if form.validate_on_submit():
        ...
        return redirect(url_for('index'))
    return render_template("register.html", title="Employee Register", form=form, register=True)
####################################################################################################################################

@app.route('/register_as_professional', methods =['GET', 'POST'])
def register_as_professional():
    ...
    if form.validate_on_submit():
        ...
        return redirect(url_for('index'))
    return render_template("register_as_professional.html", title="Register Professional", form=form, register_as_professional=True)
...
def adminregister():
    ...
    if form.validate_on_submit():
        ...
        return redirect(url_for('index'))
    return render_template("register.html", title="Manager Register", form=form)
...
def cv_selection():
        ...
        return render_template('cv_selection.html',files_list=files_list,cv_selection=True)



@app.route('/cv_selection_evaluate',methods=['GET','POST'])
...
# To check allowed extensions of uploaded resumes
def allowed_file(filename):
    return '.' in filename and \
           filename.rsplit('.', 1)[1].lower() in ALLOWED_EXTENSIONS


@app.route('/job_application_submit',methods=['GET','POST'])
...
def job_application_submit():
    ...
    return render_template('submit.html',data={'name':username,'phone':phone1,'email':email,'filename':filename})



####################################################################################################################################################

@app.route('/project_undertakings',methods=['GET','POST'])
...
def project_undertakings():
    ...
    return render_template('project_undertakings.html',project_undertakings=True,project_list=project_list)

############################################################################################################################################################
@app.route('/employee_project_details',methods=['GET','POST'])
...
def employee_project_details():
    ...
    return render_template('employee_project_details.html',employee_project_details=True,project_list=project_list)


@app.route('/display_employees',methods=['GET','POST'])
